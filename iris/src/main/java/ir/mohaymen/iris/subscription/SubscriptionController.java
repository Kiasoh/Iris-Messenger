...
public class SubscriptionController extends BaseController {
    ...
    public ResponseEntity<GetChatDto> addToChat(@RequestBody @Valid AddSubDto addSubDto) {
        ...
        for (Long id : addSubDto.getUserIds())
            subscriptionService.createOrUpdate(new Subscription
                    (null, userService.getById(id),
                    chat, chat.getMessages().get(chat.getMessages().size() - 1).getMessageId(),Permission.getDefaultPermissions(chat.getChatType())));
